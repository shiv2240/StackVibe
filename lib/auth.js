...
class AuthManager {
   ...
   */
  async register(name, email, password, role = "Developer / Designer", securityQuestion = "", securityAnswer = "") {
    if (!email || !password) {
    ...
    try {
      if (typeof ConvexClientWrapper !== 'undefined') {
        ...
        const convexUser = await convex.registerUser(user.name, email, password, role, securityQuestion, securityAnswer);
  ...
  /**
   * Fetch security question for password recovery
   */
  async getSecurityQuestion(email) {
    if (!email) throw new Error("Email address is required.");
    if (typeof ConvexClientWrapper !== 'undefined') {
      const convex = new ConvexClientWrapper();
      return await convex.getSecurityQuestion(email);
    }
    throw new Error("Convex backend client unavailable.");
  }

  /**
   * Reset user password via security answer verification
   */
  async resetPassword(email, securityAnswer, newPassword) {
    if (!email || !securityAnswer || !newPassword) {
      throw new Error("Email, security answer, and new password are required.");
    }
    if (typeof ConvexClientWrapper !== 'undefined') {
      const convex = new ConvexClientWrapper();
      return await convex.resetPassword(email, securityAnswer, newPassword);
    }
    throw new Error("Convex backend client unavailable.");
  }
